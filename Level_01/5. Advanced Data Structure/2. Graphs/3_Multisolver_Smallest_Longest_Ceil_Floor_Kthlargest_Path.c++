...
We are simply doing a DFS traversal, hence the time complexity will be O(V + E) where V = number of vertices in the graph and E = number of edges in the graph.


Space Complexity : O(V)

To perform BFS, we use priority queue data structure to find kth largest path and a visited array, which will take O(V) space, where V is the number of vertices in the graph.*/