...
void displayArrRev(int* arr, int n, int idx) {
    if (idx == n) 
      return;
    displayArrRev(arr, n, idx + 1);
    cout<<arr[idx]<<endl;
}
...
int main(){
    ...
    displayArrRev(arr, n, 0);
}

/*Time Complexity : O(n)

Let's try to understand our time complexity. It will be similar to the previous problem.
...
Although we are not using any array the space complexity will still be O(n).*/